...
| **Export** | Download your collection as CSV with `status` (`collected`, `ordered`, or `want`) and `want_rank` for want-list priority. **Import collection CSV** replaces collected, on-order, and want membership plus want order for the active storage mode only — export then re-import round-trips exactly (legacy 3- and 4-column files still import; missing ranks use row order among want rows) |
...
5. **Import collection CSV** / **Export collection CSV** (gear → About): export writes `title,author,year,status,want_rank` for every collected, on-order, and wanted title (`want_rank` is 1-based priority; blank for non-want rows). Import replaces all three lists and want priority for the **active storage option only** (this device and GitHub Gist keep separate collections). Re-importing an exported file restores the same membership and want order. Older CSVs without `want_rank` still import; want rows without a rank keep file order. Legacy 3-column CSVs (no status column) still import every row as collected. Works on this device or with GitHub Gist sync (Gist upload happens immediately when connected).
...
**`npm run build`** — writes `build/` for static hosting (`READ_ONLY`, bundled CSS/JS, compact/minified JS in `build/js/` and `build/data/`, `noindex`). Cover images in `covers/` are resized to WebP at build time (480px card thumbnails, 960px detail images, and 1200×80 list strips); masters stay in the repo but only optimized files ship in `build/`. List strip crops use per-book focal points from edits when set (default center / 70% down).
